...
class Blockchain {
     ...
     *- ...At the end you need to create the `block hash` and push the block into the chain array. Don't for get 
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if(timeElapsed < 5 * 60 * 1000){
                ...
                reject('Too much time has passed');
            }
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
            let chainEntry = self.chain.filter(function(entry){
                return entry.hash === hash;               
            });

            if(chainEntry.length !== 0){
                resolve(chainEntry[0]);
            }else{
